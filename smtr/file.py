...
def find_files(path, suffix, relative=None):
    root_dir = os.getcwd()
    input_path = os.path.join(root_dir, path)
    files = os.listdir(input_path)
    if relative:
        return [os.path.join(path, file) for file in files if file[-len(suffix):] == suffix]
    else:
        return [os.path.join(input_path, file) for file in files if file[-len(suffix):] == suffix]
